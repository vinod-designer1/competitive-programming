...
function set_jwplayer_events(jw_player,container_id,start){
     jw_player.onReady(function() {	current_position = 0;	                                		
                                    ...
                                    jw_player.onTime(function(event){timeMonitor(event);});										
								   });        	 
       }
function get_caption_details(container_id) { 
	...
	if(index > -1 ) {
		container_id = container_id.substring(0, index ? index :container_id.length);
		}
		if(document.getElementById("caption_" + container_id)!= null){
			...
			var caption_url = setMultipleCaptions(captions_info.replace(/'/ig,'"'));
			return caption_url;
	}
	return ;
}
