...
def printclust(clust, labels=None, n=0):
	# indent to make a hirerachy layout
	for i in range(n): print ' ',
	if clust.id < 0:
		# negative id means that this is an branch
		print '-'
	else:
...
def getheight(clust):
	# Id this an endpoint? Then the height is just 1
...
def getdepth(clust):
	# The ditance of an endpoint is 0.0
	if clust.left == None and clust.right == None:
		return 0
...
def drawnode(draw, clust, x, y, scaling, labels):
	if clust.id < 0:
		...
		# If this is an endpooint, draw the item label
		draw.text((x+5, y-7), labels[clust.id], (0,0,0))

def drawdendogram(clust, labels, jpeg='clusters.jpeg'):
	# height and width
...
def kcluster(rows, distance=pearson, k=4):
    ...
    # Create k randonly placed centroids
