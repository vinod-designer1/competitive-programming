...
class FlipPredictor(object):
    def __init__(self,coins):
        self.coins=coins
        n=len(coins)
        self.probs=[1/(n*1.0)]*n
    
    def pheads(self):
        # Write a function that returns 
        # the probability of the next flip being heads
        phead = 0 
        # Loop over self.coins (P(H|C)) and self.probs(P(C))
        # and calculate P(H):
        for (c,b) in zip(self.coins, self.probs):
            phead = phead + c*b  
        return phead
    ...
    def update(self,result):
        ...
        # Calculate P(C|H) = P(H|C) * P(C) / P(H) if result is H
        # Calculate P(C|T)  = P(T|C) * P(C) / P(T) if result is T
        # P(T|C) = 1 - P(H|C)
        # P(C|H) and P(C|T) is the update of P(C) on every flip 
        if result == 'H':
           phead = 0
           for (c,b) in zip(self.coins, self.probs):
              phead = phead + c*b
